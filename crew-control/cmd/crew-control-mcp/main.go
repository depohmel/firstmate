...
// Usage:
//
//	crew-control-mcp --fm-home /path/to/firstmate --bind 192.168.1.10 --port 3001
//
// For local testing:
//
...
import (
	...
	"github.com/depohmel/firstmate/crew-control/internal/crewcrtl"
...
func run() error {
	fmHome := flag.String("fm-home", crewcrtl.DefaultFirstMateHome,
		"firstmate home directory (parent of bin/, data/, state/). "+
			"Falls back to the FM_HOME environment variable if not set.")
	...
	if *fmHome == "" {
		return fmt.Errorf("--fm-home is required (or set FM_HOME environment variable)")
	}
