...
A **fork-local**, read-only HTTP MCP server that
...
> famclaw is the MCP client that calls this server at runtime via:
...
- `--fm-home` defaults to the `FM_HOME` environment variable (override if your firstmate
  home is elsewhere, or set it explicitly with `--fm-home`).
