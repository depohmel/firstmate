...
exclusion_reason() {
  case "$1" in
    ...
    fm-backend.test.sh)
      ...
      ;;
    fm-spawn-dispatch-profile.test.sh|fm-spawn-worktree-lease.test.sh|fm-trace-context-spawn.test.sh)
      printf '%s\n' 'real isolated git worktrees plus spawn lease acquisition; gray zone until dedicated proof'
      ;;
