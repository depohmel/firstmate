...
# worktree itself with `treehouse get --lease` and only navigates the pane into
# it with the send primitive below; it no longer runs `treehouse get` inside the
# pane and polls the current-path primitive to discover the path.
#
