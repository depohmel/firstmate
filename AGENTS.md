...
For ship and scout tasks, the script itself acquires the worktree with a durable `treehouse get --lease` (recording the leased path as `worktree=`, released on abort and by teardown), then navigates the pane into it; Orca creates an Orca-owned worktree, validates it, then creates the terminal.
