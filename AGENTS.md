...
state/               volatile runtime signals; gitignored
  ...
  AFK-WEDGED.txt     top-level loud alarm written when hard-ceiling force-inject still cannot land; cleared on a successful flush
...
For `kind=secondmate`, the same script launches in the registered or explicit firstmate home instead of leasing a treehouse worktree for a project, records `home=` and `projects=`, and uses the charter brief as the launch prompt.

For ship and scout tasks, tmux/herdr/zellij/cmux create a runtime endpoint and the script itself acquires the worktree with a durable `treehouse get --lease` (recording the leased path as `worktree=`, released on abort and by teardown), then navigates the pane into it; Orca creates an Orca-owned worktree, validates it, then creates the terminal. In all cases, the script asserts the resolved worktree is a genuine isolated worktree distinct from the primary checkout (aborting the spawn otherwise, to prevent the worktree tangle of section 8), installs the turn-end hook, records `state/<id>.meta`, and launches the agent with the brief.
