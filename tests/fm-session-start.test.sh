...
EOF
  ...
  # Use treehouse (a firstmate-specific binary that is never a system package)
  # rather than a common tool like node, so removing the fake stub reliably
  # yields a MISSING line even on a host whose base PATH already has node/gh/tmux.
...
EOF
  make_fake_toolchain "$fakebin"
  make_fake_ps_claude "$fakebin"
  # treehouse (firstmate-specific) is reliably absent from a bare system PATH;
  # node/gh/tmux may already be installed on the host and would defeat this.
  rm -f "$fakebin/treehouse"
