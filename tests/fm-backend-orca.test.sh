...
test_spawn_releases_orca_resources_when_metadata_write_fails() {
  ...
  # The state override points at a regular file, so `mkdir -p "$STATE"` fails.
  # Accept either coreutils flavor's wording (GNU: "File exists",
  # uutils: "Already exists") so the check is portable across mkdir implementations.
  case "$out" in
    *"File exists"*|*"Already exists"*) : ;;
    *) fail "spawn should fail at the state directory creation point: $out" ;;
  esac
