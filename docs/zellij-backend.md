...
This op was written to serve `fm-spawn.sh`'s own worktree-discovery poll loop, its only caller - injecting a harmless extra cwd probe into the pane's scrollback before the harness ever launched, an acceptable trade for a reliable answer that did not affect the interactive session the crewmate later runs in.
`fm-spawn.sh` now acquires the worktree with a direct `treehouse get --lease` instead of polling the pane, so the primitive currently has no live caller; it is retained as the backend's cwd-read primitive, and any future caller needing a live cwd read would face the same subshell-tracking constraint it works around.
...
- **The `pwd`-probe workaround for worktree-path discovery was written for `fm-spawn.sh`'s own poll loop** (see "Worktree-path discovery" above); `fm-spawn.sh` now leases the worktree directly via `treehouse get --lease` and no longer calls it, so it currently has no live caller. It is not a general-purpose live-cwd primitive; a future caller needing a live cwd read for a zellij pane would need the same active-probe approach, not a passive JSON field.
