...
Herdr teardown closes that pane before the treehouse worktree return: `treehouse return` kills every lingering process in the worktree, which includes the pane's own shell, and when that kill reaches the Herdr server first, Herdr drops the workspace through its own pane-death path and clamps the active workspace index to a stale value.
