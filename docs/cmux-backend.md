...
- **`fm_backend_cmux_current_path` has no live caller** - the worktree-discovery poll that once used it was replaced by the `treehouse get --lease` approach (see `bin/fm-spawn.sh`), so this probe primitive exists but is not invoked by any current code path.
