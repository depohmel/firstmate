...
It owns task endpoint creation, bounded capture, text/key sends, current-path reads, live-window fallback lookup, agent-process liveness probes where verified, and endpoint teardown.
...
For those pooled backends, `fm-spawn.sh` acquires the worktree by calling `treehouse get --lease --lease-holder fm-<id>` itself and records the leased path treehouse prints as the task's `worktree=`, so the task record always reflects the path treehouse actually leased rather than a cwd observed in the pane.
The lease is durable, so it is released on a spawn abort and by teardown's existing `treehouse return --force`.
